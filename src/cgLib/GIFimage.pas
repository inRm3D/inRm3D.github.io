...
{ GIFDelayExp:
  ...
  Since our paint routines are much faster than Mozilla's, you might need to
  increase this value if your animations loops too fast. The optimal value is
  impossible to determine since it depends on the speed of the CPU, the viceo card,
...
{$ENDIF}
{ GIFMinimumDelay:
 The minumum delay used in the Mozilla source is 10mS. This corresponds to a
 value of 1. However, since our paint routines are much faster than Mozilla's,
 a value of 3 or 4 gives better results.
}
  GIFMaximumDelay: integer = 1000;
{ GIFMaximumDelay:
 Maximum delay when painter is running in main thread (goAsync is not set).
 This value guarantees that a very long and slow GIF does not hang the system.
